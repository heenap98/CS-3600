...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
     ...
     #    print("Noisy Distance: " + str(noisyDistance))
        # print("Emission model: " + str(emissionModel))
        # print("Pacman position: " + str(pacmanPosition))
        allPossible = util.Counter()
        # print(type(allPossible))
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:
                    # B'(xt + 1) = Sum of all xt such that P(noisyDistance | TrueDistance or xt) * B(xt)
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                ...
                # print("Prob: " + str(prob))
                #B'(t + 1) = Sum for all xt such that Pr(ghost is at position p at time t + 1 | ghost is at position p at time t) * B(p)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        #print("Legal positions: " + str(self.legalPositions))
        for numPart in range(self.numParticles):
            particlePosition = self.legalPositions[numPart % len(self.legalPositions)]
            self.particles.append(particlePosition)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            #Ghost has been captured by PacMan
            for numPart in range(self.numParticles):
                #All particles should be updated
                self.particles[numPart % len(self.particles)] = self.getJailPosition()
        
        #Noisy distance is not None
        else:
            #weight = Sum of emissionModel
            ...
            #Check if particles received 0 weight
            if particleWeights.totalCount() == 0:
                #Resample particles uniformly at random from self.legalPositions
                self.particles = []
                for numPart in range(self.numParticles):
                    particle = random.choice(self.legalPositions)
                    self.particles.append(particle)
            #Else particles did not received 0 weight
            else:
                for numPart in range(self.numParticles):
                    self.particles[numPart % len(self.particles)] = util.sample(particleWeights)
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            #Obtain distribution over new positions for each ghost
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            #Randomly sample from each ghost's distribution
    ...
    def getBeliefDistribution(self):
        ...
        #Keep track of how many times sample appears in self.particles in beliefDistribution
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        listOfParticles = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(listOfParticles)
        for numPart in range(self.numParticles):
            particle = listOfParticles[numPart % len(listOfParticles)]
            self.particles.append(particle)
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        #Handle case if ghost is captured by PacMan
        for i in range(self.numGhosts):
            if noisyDistances[i] is None:
                #Ghost has been captured by PacMan
                for numPart in range(self.numParticles):
                    #All particles should be updated
                    # self.particles[numPart % len(self.particles)][i] = self.getJailPosition(i)
                    self.particles[numPart % len(self.particles)] = self.getParticleWithGhostInJail(self.particles[numPart % len(self.particles)], i)


        #Calculate weights of all ghosts
        particleWeights = util.Counter()
        
        for part in self.particles:
            particleWeight = 1.0
            for i in range(self.numGhosts):
                #Multiply weights for each ghost
                if noisyDistances[i] is not None:
                    trueDistance = util.manhattanDistance(pacmanPosition, part[i])
                    particleWeight *= emissionModels[i][trueDistance]
            #Add weight for ghost to current weight
            particleWeights[part] += particleWeight
        ...
        if particleWeights.totalCount() == 0:
            ...
            #Check if noisy distances are None to put ghosts in jail
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    #Ghost has been captured by PacMan
                    for numPart in range(self.numParticles):
            ...
            for numPart in range(self.numParticles):
                self.particles[numPart] = util.sample(particleWeights)
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            #Same as elapse time from before
            for i in range(self.numGhosts):
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        #Same belief distribution as approximate filtering
        #Only change is keys of dictionary are tuple of tuples due to multiple ghosts

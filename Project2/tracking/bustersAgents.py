...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # print("Living ghosts: " + str(livingGhosts))
        # print("Position distributions: " + str(livingGhostPositionDistributions))
        # print("")

        #Use argMax() on util.Counter object (items inside list livingGhostPositionDistributions) to find closest ghost
        count = 0
        for livingGhost in livingGhostPositionDistributions:
            if count == 0:
                livingGhostPWithHighestProbPosition = livingGhost.argMax()
                highestProb = livingGhost[livingGhostPWithHighestProbPosition]
                count += 1
            else:
                positionToCheck = livingGhost.argMax()
                probabilityToCheck = livingGhost[positionToCheck]
                if probabilityToCheck > highestProb:
                    livingGhostPWithHighestProbPosition = positionToCheck
                    highestProb = probabilityToCheck

        #Find action to closest ghost position
        count = 0
        for legalAction in legal:
            #Calculate new potential position for each action
            successorPosition = Actions.getSuccessor(pacmanPosition, legalAction)

            #Calculate distance for each new potential position from each action.
            #If minimum distance, that is best action.
            distance = self.distancer.getDistance(successorPosition, livingGhostPWithHighestProbPosition)
            if count == 0:

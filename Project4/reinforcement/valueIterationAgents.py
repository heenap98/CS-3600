...
class ValueIterationAgent(ValueEstimationAgent):
            ...
            if not (mdp.isTerminal(state)):
              ...
              for a in possibleActions:
                ...
                if count == 0:
                  ...
                  actionVal = max(QValue, actionVal)
              self.values[state] = actionVal
          #Update old values based on values
          self.oldValues = self.values.copy()
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for i in self.mdp.getTransitionStatesAndProbs(state, action):
          ...
          # print(state)
          # print(prob)
          y = ( self.discount * self.oldValues[newState] ) + self.mdp.getReward(state, action, newState)
          QValue = QValue + (prob * y)
    ...
    def computeActionFromValues(self, state):
        ...
        if len(possibleActions) == 0:
          return None
        for a in possibleActions:
          aVal = self.computeQValueFromValues(state, a)
          if count == 0:
            maxVal = aVal
            finalAction = a
            count = count + 1
